...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        data = filtered_df.groupby(['Launch Site'])['class'].sum().to_frame()
        fig = px.pie(data, values='class',
        names= ['CCAFS LC-40', 'CCAFS SLC-40', 'KSC LC-39A', 'VAFB SLC-4E'],
        title='Total Success Launches By Site')
        fig = fig.update_traces(textposition='inside', textinfo='percent+label+value')
        return fig
    else:
        filtered_df = filtered_df[filtered_df['Launch Site'] == entered_site]
        data = filtered_df.groupby(['class'])['class'].count().to_frame()
